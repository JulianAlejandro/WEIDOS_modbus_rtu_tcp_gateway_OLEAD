...
//////AMR - Define Modbus request structure
typedef struct modbus_request_struct_t{
  uint8_t modbusId;
...
typedef struct {
  ...
  byte serialAttempts;
  byte serverAddress; //Modbus Server Address
} config_t;
...
void setup() {
  Serial.begin(115200);
  watchDog.attachShutdown(shutdownPrint);
  watchDog.setup(WDT_HARDCYCLE16S);  // initialize WDT-softcounter refesh cycle on 32sec interval
  Ethernet.init(ETHERNET_CS);
  E2PROM.begin();
  E2PROM.get(DATA_START, data);
...
//AMR 
//Detecion flanco para eascribir que Webserver esta activo en OLEAD
...
void loop() {
  ...
  if (digitalRead(DI_7)== LOW)
  {
    checkOfflineStatus();
    checkInternalPolling(); // frecuencia de actuacion de 5 segundos 
    updateOLEAD();
    waitingShown = false; //reset flanco 
  } else {
    // Print OLEAD "Webserver On"
   if (!waitingShown) {   // Solo entra la primera vez
      drawWaitingOLEAD();
      waitingShown = true;
    }
