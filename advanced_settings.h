...
const int DEFAULT_SERVER_ADDRESS = 10;
/****** ADVANCED SETTINGS ******/
...
#define RS485_CONTROL_PIN RTE_DE              // Arduino Pin for RS485 Direction control, disable if you have module with hardware flow control
...
const byte ETH_RESET_PIN = 0;                    // Ethernet shield reset pin (deals with power on reset issue on low quality ethernet shields)
...
const uint16_t STATUS_FETCH_INTERVAL = 500;     // Fetch API interval (ms) for the Modbus Server configuration webpage to renew data from JSON served by Arduino
